...
class Common{
	...
	public static function getSignFromApk($file) {
		...
		$proc = proc_open(dirname(__DIR__).'/../public/apks/apks '.$file. ' 2>/dev/null|grep META-INF', $descriptorspec, $pipes, realpath(dirname(realpath(__FILE__)).'/../config/gnu'));
    ...
    public static function get_heifer_url(){
    	if($_SERVER['SERVER_ADDR'] == '127.0.0.1' || $_SERVER['SERVER_NAME'] == "ssp.dev.anzhi.com"){
    ...
    //提现申请
    public static function put_forward($map){
    	$host= self::get_heifer_url();
		$url = '/http/ad/add/applyReq.json';
		$data = array(
				'serviceId'	=>	'047',
    ...
    //账单下载通用接口
    public static function bill_download($map){
    	$host= self::get_heifer_url();
		$url = '/advertisingMediaSettlement/get/export.json';
		if($map['type'] == 1) {
			$filename = "月度账单.pdf";
			$param = array(
				'id'		=>	$map['id'],
				'muid'		=>	$map['muid'],
				'muidType'	=>	$map['muidType'],
			);
			if($map['muidType'] == 1) {
				$exeType = 1;
			}elseif($map['muidType'] == 2){
				$exeType = 3;
			}
		}else {
			$filename = "结算确认函.pdf";
		...
		$data = array(
				...
				'excType'	=>	$exeType,
				'data'		=>	self::encode($param),
		);
		$vals	=	http_build_query($data);
		$ret = self::httpGetInfo($host.$url, $vals);
		echo $ret;
