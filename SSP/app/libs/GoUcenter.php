...
class GoUcenter{
	...
	private $config = array(
		...
		'serviceId' 		=>	'047',
		'serviceVersion'	=>	'1.0',
		'privatekey'		=>	'L1BU6H306k2BDTunJ8cPHmPU',//测试环境
		//'privatekey'		=>	'6J1CsK70SH8b8Fb6K3dM8Is2',//线上
		'serviceType'		=>	1,
	);
	...
	function token_userinfo(){
		...
		$data = array();
		$loginIp = isset($userinfo['loginip'])?$userinfo['loginip']:'';
		if(isset($ucinfo['sid'])){
			$data['sid'] = $ucinfo['sid'];
			$data['pid'] = $ucinfo['pid'];
		}
		...
		if(!empty($data)){		    
			...
			if( $code == 200){
				...
				$info = array(
					...
					'MOBILE' 	=> isset($userinfo['telephone'])?$userinfo['telephone']:'',
				);
				return $info;
			}
		}
