...
$di -> setShared('db2', function () use($config, $di) {
    $dbconfig = $config -> db2 -> db;
    $dbconfig = $dbconfig -> toArray();
    if (!is_array($dbconfig) || count($dbconfig)==0) {
        throw new \Exception("the db2 config is error");
    }
...
$di -> setShared('redis', function() use($di){
    $redis_config = array(
            'host'  =>  'honorm81.redis.lan.anzhi.com',
            'port'  =>  6381,
            'auth'  => 'az88Ai',
        );
    $redis = new \Marser\App\Libs\Redis($di, $redis_config);
    return $redis;
});
